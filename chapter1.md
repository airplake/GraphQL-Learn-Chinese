...
至此，所有查询语句里面的变量都已经被写到了。但是在大多数应用中，fields中的变量将会是动态的：比如，可能有一个下拉菜单让你选择你喜欢的星战集数，或者一个搜索field，或者一组过滤。

将所有动态变量直接通过查询语句直接传入服务端并不是一个好的想法，因为之后客户端的代码将需要在运行时（runtime）动态的操作查询语句，然后将它序列化到GraphQL特定的模式。相反，GraphQL有一个最棒的方式来独立于语句本身操作动态数据，然后将它们作为不同的字典传送。这些数据被叫做 `变量`。

当我们开始着手变量的时候，我们需要做三件事：

1. 用 $variableName 替换语句中的所有静态变量
